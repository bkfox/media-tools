...
class AppMeta(type):
    ...
    def init_actions(mcls, cls):
        ...
        cls.actions.update(
            (name, action) for name, action in callables
...
class App(metaclass=AppMeta):
    ...
    def run(self, **context):
        """By default, lookup for actions, and run them all by order of declaration. """
...
class FilesApp(App):
    """ Application taking input files as positional argument.
    Provide utilities to work with it:
    - command line argument `sources`;
    - context `files`: files read from sources
    - save method.
    """
    read_files_from = ("sources",)
    """ Tuple of variable names to read one ``get_context``.
    Tuple values can be:
    ...
    """ Files will be passed to context with this parameter name, as a dict of ``{path: content}``."""

    def init_parser(self, parser):
        super().init_parser(parser)
        parser.add_argument("sources", nargs="+", type=Path, metavar="SOURCES",
            help="Input files.")

    def get_context(self, **kwargs):
        """
        Remove missing files from sources and read files (returned as context
        ``files`` value).
        """
        ...
        if missings:
            ...
            self.logs.warn(
                f"Following sources are missing and wont be proceed:\n{lst}"
            )
        ...
        return super().get_context(sources=sources, **kwargs)
